 ...
 * {@link com.vldocking.swing.docking.ui.DockViewUI} delegate.
 * <p>
...
public class DockView extends LockableUnlockableHeaderPanel implements DockDropReceiver, SingleDockableContainer {
	...
	protected void scanDrop(DockEvent event, boolean drop) {
		...
		if(! accepted) {
			...
			if(drop) {
				event.setDockingAction(new DockingActionCreateTabEvent(event.getDesktop(), d, dockableLocation, viewLocation, dockable, 0));
