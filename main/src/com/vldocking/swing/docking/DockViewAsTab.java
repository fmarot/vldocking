...
public class DockViewAsTab extends DockView implements SingleDockableContainer {
	...
	protected void initMaximizedPopUp(JPopupMenu popup) {
		...
		popup.add(createPopUpItem(RESTORE_TEXT, restoreIcon, RESTORE_TEXT, "maximize", (KeyStroke) UIManager.get("DockingDesktop.maximizeActionAccelerator")));
	}
	...
	protected void initAutoHidePopUp(JPopupMenu popup) {
		if(DockingUtilities.isChildOfCompoundDockable(getDockable())) {
			...
			popup.add(createPopUpItem(RESTORE_TEXT, dockIcon, RESTORE_TEXT, "dock", (KeyStroke) UIManager.get("DockingDesktop.dockActionAccelerator")));
		}
		if(getDockable().getDockKey().isCloseEnabled()) {
			popup.add(createPopUpItem(CLOSE_TEXT, closeSmartIcon, CLOSE_TEXT, "close", (KeyStroke) UIManager.get("DockingDesktop.closeActionAccelerator")));
		}
	}
	...
	protected void initDockedPopUp(JPopupMenu popup) {
		...
		if(key.isAutoHideEnabled()) {
			popup.add(createPopUpItem(ICONIFY_TEXT, hideSmartIcon, ICONIFY_TEXT, "dock", (KeyStroke) UIManager.get("DockingDesktop.dockActionAccelerator")));
		}
		if(key.isFloatEnabled()) {
			popup.add(createPopUpItem(FLOAT_TEXT, floatSmartIcon, FLOAT_TEXT, "float", (KeyStroke) UIManager.get("DockingDesktop.floatActionAccelerator")));
		}
		if(key.isMaximizeEnabled()) {
			popup.add(createPopUpItem(MAXIMIZE_TEXT, maximizeSmartIcon, MAXIMIZE_TEXT, "maximize", (KeyStroke) UIManager.get("DockingDesktop.maximizeActionAccelerator")));
		}
	...
	protected void initFloatingPopUp(JPopupMenu popup) {
		if(DockingUtilities.isChildOfCompoundDockable(getDockable())) {
			...
			popup.add(createPopUpItem(ATTACH_TEXT, attachIcon, ATTACH_TEXT, "float", (KeyStroke) UIManager.get("DockingDesktop.floatActionAccelerator")));
		}
	}
	...
	protected void configureMaximizeButton() {
		if(isMaximizeButtonDisplayed) {
			...
			// add a tooltip
			maximizeAction.putValue(AbstractAction.SHORT_DESCRIPTION, UIManager.get("DockViewTitleBar.maximizeButtonText"));
		}
