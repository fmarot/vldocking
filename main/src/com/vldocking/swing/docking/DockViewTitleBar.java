 ...
 * <p>
 * Note : the UI Delegate of the DockViewTitleBar is the {@link com.vldocking.swing.docking.ui.DockViewTitleBarUI}
 *
...
public class DockViewTitleBar extends JPanel implements DockableDragSource {
	...
	protected void initAutoHidePopUp(JPopupMenu popup) {
		...
		if(target.getDockKey().isCloseEnabled()) {
			popup.add(createPopUpItem(CLOSE_TEXT, closeIcon, CLOSE_TEXT, "close", (KeyStroke) UIManager.get("DockingDesktop.closeActionAccelerator")));
		}
	}
	...
	protected void initDockedPopUp(JPopupMenu popup) {
		...
		if(key.isAutoHideEnabled()) {
			popup.add(createPopUpItem(ICONIFY_TEXT, hideIcon, ICONIFY_TEXT, "dock", (KeyStroke) UIManager.get("DockingDesktop.dockActionAccelerator")));
		}
		if(key.isFloatEnabled()) {
			popup.add(createPopUpItem(FLOAT_TEXT, floatIcon, FLOAT_TEXT, "float", (KeyStroke) UIManager.get("DockingDesktop.floatActionAccelerator")));
		}
		if(key.isMaximizeEnabled()) {
			popup.add(createPopUpItem(MAXIMIZE_TEXT, maximizeIcon, MAXIMIZE_TEXT, "maximize", (KeyStroke) UIManager.get("DockingDesktop.maximizeActionAccelerator")));
		}
